"""Worker → Spine: events, leaves, heartbeat"""
...
class EventIn(BaseModel):
    call_id: str = ""; phone_id: Optional[str] = None; event_type: str = ""
    step_id: Optional[str] = None; step_type: Optional[str] = None
    data: Optional[Any] = None; timestamp: str = ""

class LeafIn(BaseModel):
    leaf_id: str = ""; call_id: str = ""; step_id: str = ""; type: str = ""
    content: Optional[str] = None; wa_type: Optional[str] = None
    status: str = "Pending"; timestamp: Optional[str] = None; meta: Optional[dict] = None

class LeafStatusIn(BaseModel):
    status: str; message_id: Optional[str] = None

class HeartbeatIn(BaseModel):
    phone_id: str; service_name: str; port: int = 9000; status: str = "online"
...
def ingest_event(ev: EventIn):
    ...
    db.table("spine_events").insert({
        ...
        "data": ev.data, "timestamp": ev.timestamp or _now(),
    }).execute()
    return {"ok": True}
...
def ingest_leaf(leaf: LeafIn):
    ...
    db.table("spine_leaves").insert({
        ...
        "status": leaf.status, "timestamp": leaf.timestamp or _now(), "meta": leaf.meta,
    }).execute()
    return {"ok": True}
...
def update_leaf(leaf_id: str, body: LeafStatusIn):
    ...
    if body.message_id:
        res = db.table("messages").select("id").eq("whatsapp_message_id", body.message_id).limit(1).execute()
        if res.data:
            try: db.table("spine_leaf_messages").insert({"leaf_id": leaf_id, "message_id": res.data[0]["id"]}).execute()
            except: pass
    return {"ok": True}
